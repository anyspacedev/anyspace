...
export async function launchTeam(
  ...
  for (const ta of teamAgents) {
    ...
    const env: Record<string, string> = {
      ...plan.env,
      ...apiEnv,
      ANYSPACE_TASK_FILE: promptFile.path,
      ANYSPACE_TEAM_DIR: paths.teamDir,
      ANYSPACE_TEAM_ID: team.id,
      ANYSPACE_TEAM_NAME: team.name,
      ANYSPACE_AGENT_LABEL: ta.label,
      ANYSPACE_AGENT_ROLE: ta.role,
      ANYSPACE_AGENT_ID: ta.id,
      ANYSPACE_BOARD_PATH: paths.boardPath,
      ANYSPACE_MESSAGES_PATH: paths.messagesPath,
      ANYSPACE_TEAM_TMSG: paths.tmsgPath,
      ANYSPACE_TEAM_BIN_DIR: paths.tmsgBinDir,
    };
...
export async function addAgentToLiveTeam(
  ...
  const preset: PanePreset = {
    ...
    spawnEnv: {
      ...
      ANYSPACE_TEAM_ID: team.id,
      ANYSPACE_TEAM_NAME: team.name,
      ANYSPACE_AGENT_LABEL: args.label,
      ANYSPACE_AGENT_ROLE: args.role,
      ANYSPACE_AGENT_ID: newTa.id,
      ANYSPACE_BOARD_PATH: paths.boardPath,
      ANYSPACE_MESSAGES_PATH: paths.messagesPath,
      ANYSPACE_TEAM_TMSG: paths.tmsgPath,
      ANYSPACE_TEAM_BIN_DIR: paths.tmsgBinDir,
  ...
  if (newPane && newPane.kind === "terminal") {
    ...
    useWorkspaceStore.getState().setPanePayload(team.tabId, newPaneId, {
      ...
      spawnEnv: {
        ...existingEnv,
        ANYSPACE_PANE_ID: newPaneId,
        ANYSPACE_TAB_ID: team.tabId,
      },
    });
  }
