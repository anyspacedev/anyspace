...
/// `$ANYSPACE_USER_ZDOTDIR`) and then source the integration script.
...
if [ -n "$ZSH_VERSION" ]; then
  ...
  add-zsh-hook precmd __anyspace_precmd 2>/dev/null
  add-zsh-hook preexec __anyspace_preexec 2>/dev/null
elif [ -n "$BASH_VERSION" ]; then
  __anyspace_last_exit=0
  __anyspace_prompt_command() {
    __anyspace_last_exit=$?
    __anyspace_emit_d "$__anyspace_last_exit"
    __anyspace_emit "A"
  }
  __anyspace_debug_trap() {
  ...
  PROMPT_COMMAND="__anyspace_prompt_command;${PROMPT_COMMAND:-}"
  trap '__anyspace_debug_trap' DEBUG
fi
...
if [ -n "${ANYSPACE_TEAM_BIN_DIR:-}" ] && [ -d "$ANYSPACE_TEAM_BIN_DIR" ]; then
  case ":$PATH:" in
    *":$ANYSPACE_TEAM_BIN_DIR:"*) ;;
    *) export PATH="$ANYSPACE_TEAM_BIN_DIR:$PATH" ;;
  esac
fi
...
if [ -n "$ANYSPACE_TEAM_TMSG" ] && [ -f "$ANYSPACE_TEAM_TMSG" ]; then
  # shellcheck disable=SC1090
  source "$ANYSPACE_TEAM_TMSG"
fi
...
const ZSH_WRAPPER_ZSHENV: &str = r##"# AnySpace zsh wrapper — sources user's real .zshenv.
WRAPPER_ZDOTDIR="$ZDOTDIR"
USER_ZDOTDIR="${ANYSPACE_USER_ZDOTDIR:-$HOME}"
...
const ZSH_WRAPPER_ZPROFILE: &str = r##"USER_ZDOTDIR="${ANYSPACE_USER_ZDOTDIR:-$HOME}"
...
const ZSH_WRAPPER_ZSHRC: &str = r##"USER_ZDOTDIR="${ANYSPACE_USER_ZDOTDIR:-$HOME}"
[ -f "$USER_ZDOTDIR/.zshrc" ] && ZDOTDIR="$USER_ZDOTDIR" source "$USER_ZDOTDIR/.zshrc"
unset USER_ZDOTDIR
[ -n "$ANYSPACE_SHELL_INTEGRATION" ] && source "$ANYSPACE_SHELL_INTEGRATION"
"##;

const ZSH_WRAPPER_ZLOGIN: &str = r##"USER_ZDOTDIR="${ANYSPACE_USER_ZDOTDIR:-$HOME}"
...
const BASH_WRAPPER_RC: &str = r##"# AnySpace bash wrapper — replays user init then loads OSC 133 integration.
